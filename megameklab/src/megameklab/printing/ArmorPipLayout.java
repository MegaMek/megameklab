 ...
 * {@link ArmorPipLayout#addPips(PrintRecordSheet, Element, int, PrintRecordSheet.PipType, double, String, boolean)}.
 * </p>
 *
 *
 * <p>
...
class ArmorPipLayout {
     ...
     * @param alternateMethod If the armor pips should be attempted to be grouped in 5's
     */
    static void addPips(PrintRecordSheet sheet, Element group, int pipCount, PrintRecordSheet.PipType pipType,
          double strokeWidth, String fill, boolean alternateMethod) {
        if (pipCount > 0) {
            boolean multi = false;
            ...
            if (multi) {
                ...
                // Deal with rounding inaccuracies by distributing remaining pips starting with the first or removing
                // extras starting with the last.
                int i = 0;
                ...
                ArmorPipLayout layout = new ArmorPipLayout(sheet, group, pipType, strokeWidth, fill);
    ...
    static void addPips(PrintRecordSheet sheet, Element group, int pipCount, boolean alternateMethod) {
        addPips(sheet,
              group,
              pipCount,
              PrintRecordSheet.PipType.CIRCLE,
              0.5,
              PrintRecordSheet.FILL_WHITE,
              alternateMethod);
    }

    private ArmorPipLayout(PrintRecordSheet sheet, Element group, PrintRecordSheet.PipType pipType, double strokeWidth,
          String fill) {
        this.sheet = sheet;
        this.group = group;
     ...
     *
     * @return The dimensions of the section of the row to leave blank or null if no gap is defined, or it is malformed.
     */
        ...
        if (null != gap) {
            ...
            try {
                if (dim.length == 2) {
                    ...
                    if (left < right && left >= bbox.left - PRECISION && right <= bbox.right + PRECISION) {
                    ...
                    logger.error("Incorrect number of parameters to {} in {}",
                          IdConstants.MML_GAP,
                          rect.getAttributeNS(null, SVGConstants.SVG_ID_ATTRIBUTE));
                }
    ...
    private void process(int pipCount) {
        // Estimate the number of rows required by finding the height of a rectangle with an area of pipCount that
        // has the same aspect ratio as the bounding box.
        ...
        // Calculate the average width of the rows in pips, but no more than the number that can fit in the average
        // row. avgHeight is the initial default cell size.
        int nCols = Math.min(pipCount / nRows, (int) (avgWidth / avgHeight));

        // If the initial number is not enough to hold, the number of pips adds more rows or columns, attempting to
        // keep close to the original aspect ratio.
        ...
        // If staggered, a half-pip offsets successive rows to allow the rows to be closer together without touching
        ...
        double yPos = Math.max(bounds.top,
              bounds.top + (bounds.height() - spacing * nRows) / 2.0 + spacing * 0.5 - radius);
        ...
        for (int r = 0; r < nRows; r++) {
            ...
            Bounds row = new Bounds(Math.max(upper.left, lower.left),
                  ...
                  yPos + spacing);
            Bounds gap = mergeGaps(row,
                  negativeRegions.get(upperEntry.getKey()),
            ...
            // If the row is split and the difference in width between the two sides is within the margin of the
            // presumed spacing, use an even number so it can be split evenly between them.
            boolean mirror = gap.width() > 0 && Math.abs((gap.left - row.left) - (row.right - gap.right)) < spacing;
    ...
    /**
     * Compares gaps in two adjacent rows and returns the union. If neither row has a gap, returns a bound of zero
     * width.
     *
    ...
    private double adjustCount(int pipCount, List<Bounds> rows, List<Bounds> gaps, List<Integer> rowCount,
        ...
        List<Integer> indices = IntStream.range(0, rows.size())
                                      .boxed()
                                      .sorted(Comparator.comparingDouble(i -> rowCount.get(i) /
                                                                                    (rows.get(i).width() -
                                                                                           gaps.get(i).width())))
                                      .toList();
        ...
        // Running count of rows that are skipped due to being full or having the minimum number. If we go through
        // all the rows and skip all of them, either shrink the pips (when adding) or remove the minimum pip
        // requirement (when subtracting), then try again.
        int skipped;
        // Keep a minimum of 1 pip per row, or 2 per split row if the parts are the same size. If this still leaves
        // us with extra pips, remove this requirement.
        ...
        do {
            ...
            while ((current != pipCount) && skipped < rows.size()) {
                ...
                if (pipCount > current) {
                    ...
                    if (change > 0 &&
                              spacing * (rowCount.get(index) + change) <=
                                    rows.get(index).width() - gaps.get(index).width()) {
     ...
     * @param staggered If true, the horizontal spacing will be doubly vertical.
     * @param radius    The radius of each pip.
     */
    private void drawPips(List<Bounds> rows, List<Bounds> gaps, List<Integer> rowCount, boolean staggered,
          double radius, double xSpacing) {
        double dx = staggered ? xSpacing * 2 : xSpacing;

        // Find the row that takes up the largest percentage of its row. If it's over 100%, reduce the horizontal
        // spacing to make it fit. If lower, increase the horizontal spacing by a factor of the geometric mean of the
        // percentage and 1.0 to prevent having a cluster of pips in the center and too much blank space on the sides.
        double pct = 0.0;
        for (int r = 0; r < rows.size(); r++) {
            // Since we're measuring space between pips, only include those with multiple pips (per section for split)
        ...
        // Start by centering the top row and fit later rows into the same grid if possible. If the rows shift in a
        // way that the pips cannot fit within the grid, shift the center.
