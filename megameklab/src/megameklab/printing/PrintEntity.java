...
public abstract class PrintEntity extends PrintRecordSheet {
    ...
    /**
     * When printing from a MUL, the pilot data is filled in unless the option has been disabled. This allows a series
     * of blank record sheets to be generated without including the generated pilot data. If the crew name is "unnamed,"
    ...
    protected boolean showPilotInfo() {
        return options.showPilotData() && !getEntity().getCrew().getName().startsWith(RandomNameGenerator.UNNAMED);
    }
    ...
    protected void writeTextFields() {
        ...
        if (getEntity().getWeightClass() == EntityWeightClass.WEIGHT_SMALL_SUPPORT) {
            setTextField(TONNAGE, NumberFormat.getInstance().format((int) (getEntity().getWeight() * 1000)) + " kg");
        } else {
        ...
        // If we're using a MUL to print generic sheets, we also want to ignore any BV adjustments for C3 networks or
        // pilot skills.
        setTextField(BV,
              NumberFormat.getInstance().format(getEntity().calculateBattleValue(!showPilotInfo(), !showPilotInfo())));
        ...
        // If we need to fill in names of crew slots, we will need to reposition blanks/name fields. This will require
        // building the graphics tree so we measure the elements.
        if (getEntity().getCrew().getCrewType() != CrewType.SINGLE) {
            build();
        }
        hideUnusedCrewElements();
        for (int i = 0; i < getEntity().getCrew().getSlotCount(); i++) {
            // If we have multiple named crew for the unit, change the "Name:" label to the label of the slot. This
            // will usually require adjusting the position of the name or the length of the blank.
            double nameOffset = 0;
            ...
            if (showPilotInfo()) {
                ...
                if (spaList.length() > 0) {
                    ...
                    if (rect instanceof SVGRectElement) {
                        ...
                        addMultilineTextElement(canvas,
                              bbox.getX(),
                              bbox.getY() + lineHeight,
                              bbox.getWidth(),
                              lineHeight,
                              spaText,
                              fontSize,
                ...
                if (nameOffset != 0) {
                    ...
                    if (null != element) {
                        ...
                        element.setAttributeNS(null,
                              ...
                              String.format("M %f,%f %f,%f",
                                    rect.getX() + nameOffset,
                                    rect.getY(),
    ...
    protected void drawArmor() {
        if (getEntity().isSupportVehicle() && (getEntity().hasBARArmor(firstArmorLocation()))) {
    ...
    void drawHeatSinkPipsLandscape(Element canvas, int hsCount, double viewX, double viewY, double viewWidth,
          double viewHeight) {
    ...
    /**
     * Applies the current scale to a movement point value and adds the unit indicator. If the units are hexes, the
     * value is rounded up.
     *
    ...
    /**
     * Applies the current scale to a pair of movement point values, puts the second in brackets, and adds the unit
