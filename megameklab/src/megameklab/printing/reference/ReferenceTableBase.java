...
abstract public class ReferenceTableBase {
    ...
    public Element createTable(double x, double y, double width, double height) {
        ...
        final Element shadow = createCellBorder(2.0,
              2.0,
              width - 4.0,
              height - 4.0,
              PrintRecordSheet.FILL_SHADOW,
              PrintRecordSheet.FILL_SHADOW);
        final Element border = createCellBorder(0.0,
              0.0,
              width - 5.0,
              height - 5.0,
        ...
        g.appendChild(label);
        g.appendChild(createTableBody(3.0,
              PADDING * 1.5 + sheet.getFontHeight(FONT_SIZE_LABEL) * 2,
              width - 8.0,
              height - 15.0,
              PrintRecordSheet.FONT_SIZE_VSMALL));
        return g;
    }
    ...
    private Element createLabel(double x, double y, String title, double labelWidth) {
        ...
        background.setAttributeNS(null,
    ...
    protected Element createTextElement(double x, double y, String text, float fontSize, String fontWeight, String fill,
          String anchor, boolean fixedWidth, Double width) {
        ...
        double textLength = fontWeight.equals(SVGConstants.SVG_BOLD_VALUE) ?
                                  sheet.getBoldTextLength(text, fontSize) :
                                  sheet.getTextLength(text, fontSize);
        if (fixedWidth || ((width != null) && (textLength > width))) {
            ...
            t.setAttributeNS(null, SVGConstants.SVG_TEXT_LENGTH_ATTRIBUTE, String.valueOf(textLength));
        }
    ...
    protected Element createRoundedBorder(double x, double y, double width, double height) {
        ...
        d.add(String.format(BEZIER, 0.0, -CONTROL, RADIUS - CONTROL, -RADIUS, RADIUS, -RADIUS));
        d.add(String.format("h %f", width - RADIUS * 2));
        d.add(String.format(BEZIER, CONTROL, 0.0, RADIUS, RADIUS - CONTROL, RADIUS, RADIUS));
        d.add(String.format("v %f", height - RADIUS * 2));
        d.add(String.format(BEZIER, 0.0, CONTROL, CONTROL - RADIUS, RADIUS, -RADIUS, RADIUS));
        d.add(String.format("h %f", -width + RADIUS * 2));
        d.add(String.format(BEZIER, -CONTROL, 0.0, -RADIUS, CONTROL - RADIUS, -RADIUS, -RADIUS));
        d.add("Z");
