...
public class GroundMovementRecord extends ReferenceTableBase {
    ...
    protected Element createTableBody(double x, double y, double width, double height, float fontSize) {
        ...
        addTrack(PADDING * 1.5, PADDING, width - PADDING * 3, height * 0.3, 1, 10, g);
        addTrack(PADDING * 1.5, height * 0.5, width - PADDING * 3, height * 0.3, 11, 20, g);
        return g;
    }

    private void addTrack(double x, double y, double width, double height, int fromTurn, int toTurn, Element parent) {
        ...
        parent.appendChild(createTextElement(x + PADDING,
              startY,
              bundle.getString("turnNum"),
              fontSize,
        ...
        parent.appendChild(createTextElement(x + PADDING,
              ...
              SVGConstants.SVG_BOLD_VALUE));
        parent.appendChild(createTextElement(x + PADDING,
              height * 2.0 / rows + startY,
              bundle.getString("moveMode"),
              fontSize,
              SVGConstants.SVG_BOLD_VALUE));
        parent.appendChild(createTextElement(x + PADDING,
              height * 3.0 * rows + startY,
              bundle.getString("hexesMoved"),
              fontSize,
              SVGConstants.SVG_BOLD_VALUE));
        if (showHeat) {
            parent.appendChild(createTextElement(x + PADDING,
                  height * 4.0 * rows + startY,
                  bundle.getString("heat"),
                  fontSize,
                  SVGConstants.SVG_BOLD_VALUE));
        }
        for (int i = 0; i <= toTurn - fromTurn; i++) {
            parent.appendChild(createTextElement(startX + i * colWidth,
                  startY,
                  String.valueOf(fromTurn + i),
                  fontSize,
                  SVGConstants.SVG_BOLD_VALUE,
