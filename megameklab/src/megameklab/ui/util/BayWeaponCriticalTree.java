...
public class BayWeaponCriticalTree extends JTree {
    ...
    private TreeNode initRoot() {
        ...
        for (int eqIndex = 0; eqIndex < eSource.getEntity().getEquipment().size(); eqIndex++) {
            ...
            if (!eqSet.contains(eqIndex) &&
                      (eq.getLocation() == location) &&
    ...
    /**
     * Removes the equipment node and assigns its mount to LOC_NONE.
     *
     * @param node
     */
    ...
    private void removeEquipment(final EquipmentNode node, boolean shouldRefresh, boolean updateMount) {
        ...
        if ((node.getParent() instanceof BayNode) && (node.getParent().getParent() != null)) {
            ...
            if ((node.getMounted().getType() instanceof WeaponType) && bay.getBayWeapons().isEmpty()) {
     ...
     * @param ammo The allocated ammo to remove.
     */
    private void deleteAmmo(final AmmoMounted ammo) {
    ...
    /**
     * Node class used directly for individual mounts and served as the base class for weapon and ammo bays. Provides
    ...
    private class EquipmentNode implements MutableTreeNode {
        ...
        public int getIndex(TreeNode node) {
            if (node instanceof MutableTreeNode mutableTreeNode) {
                return children.indexOf(mutableTreeNode);
            }
        ...
        public String getTooltip() {
            ...
            if (getMounted().getType() instanceof WeaponType weaponType) {
                ...
                sb.append("<br/>AV: ")
                      .append(weaponType.getShortAV() + bonus)
                      .append("/")
                      .append(weaponType.getMedAV() + bonus)
                      .append("/")
    ...
    /**
     * Node used for weapon bays. The display name shows the current and maximum AV for the bay.
     */
    ...
    private int avMod(Mounted<?> weaponMount) {
        if ((weaponMount.getType() instanceof WeaponType) && (weaponMount.getLinkedBy() != null)) {
    ...
    private int avMod(WeaponType weapon, EquipmentType linkedBy) {
        ...
        if (linkedBy.hasFlag(MiscType.F_ARTEMIS) || linkedBy.hasFlag(MiscType.F_ARTEMIS_V)) {
            // The 9 and 10 rows of the cluster hits table is only different in column 3
                        ...
                        if (node.getMounted().getType() instanceof AmmoType at) {
                            if (node.getMounted().getBaseShotsLeft() > at.getShots()) {
                                ...
                                for (int s = at.getShots();
                                    ...
                                    info.addActionListener(ev -> deleteAmmo((AmmoMounted) node.getMounted()));
                                    delete.add(info);
                                }
    ...
    public void addToBay(@Nullable WeaponMounted bay, Mounted<?> eq) {
        ...
        if ((null == bay) || !canTakeEquipment(bay, eq)) {
            ...
            if (eq.getType().hasFlag(MiscType.F_ARTEMIS) ||
                ...
                for (WeaponMounted weapon : bay.getBayWeapons()) {
                    ...
                    if ((weapon.getLinkedBy() == null) &&
                              ((weaponType.getAmmoType() == AmmoType.T_LRM) ||
                                     (weaponType.getAmmoType() == AmmoType.T_SRM) ||
                                     (weaponType.getAmmoType() == AmmoType.T_MML) ||
                                     (weaponType.getAmmoType() == AmmoType.T_LRM_IMP) ||
                                     (weaponType.getAmmoType() == AmmoType.T_SRM_IMP) ||
                                     (weaponType.getAmmoType() == AmmoType.T_NLRM))) {
                        moveToArc(eq);
                        eq.setLinked(weapon);
                        break;
                    }
            ...
            if (eq instanceof AmmoMounted) {
                Optional<AmmoMounted> addMount = bay.getBayAmmo()
                                                       .stream()
    ...
    private void updateAmmoCapacity(AmmoMounted ammoMount) {
        // Mounted#getTonnage will give us the weight of one slot of ammo; multiply it by the number of slots to get
        // the total capacity of the ammo bin.
    ...
    private boolean canTakeEquipment(WeaponMounted bay, Mounted<?> eq) {
        if (eq.getType() instanceof WeaponType) {
            ...
            if (eq.getType().hasFlag(WeaponType.F_MASS_DRIVER)) {
                // The limit is one per firing arc; the medium and heavy exceed the 70-point limit.
                return av == 0;
                ...
                if ((weapon.getType().getAmmoType() == ((AmmoType) eq.getType()).getAmmoType()) &&
                    ...
                    return true;
                         ...
                         ((eq.getType().hasFlag(MiscType.F_ARTEMIS) || eq.getType().hasFlag(MiscType.F_ARTEMIS_V)) ||
                                eq.getType().hasFlag(MiscType.F_ARTEMIS_PROTO))) {
            for (WeaponMounted weapon : bay.getBayWeapons()) {
                final int ammoType = weapon.getType().getAmmoType();
                if ((weapon.getLinkedBy() == null) &&
            ...
            for (WeaponMounted weapon : bay.getBayWeapons()) {
                if ((weapon.getLinkedBy() == null) && (weapon.getType().getAmmoType() == AmmoType.T_MRM)) {
                    return true;
                }
            }
        } else if ((eq.getType() instanceof MiscType) &&
    ...
    public boolean isValidDropLocation(DropLocation loc, Mounted<?> eq) {
        ...
        if (null != bay) {
            if ((eq == bay) ||
                      ((eq instanceof WeaponMounted) && bay.getBayWeapons().contains(eq)) ||
