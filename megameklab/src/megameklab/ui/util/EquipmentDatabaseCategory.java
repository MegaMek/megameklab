...
public enum EquipmentDatabaseCategory {
    ...
    ARTILLERY("Artillery",
          ...
          e -> !(e instanceof ProtoMek) && (!(e instanceof Infantry) || (e instanceof BattleArmor))),

    CAPITAL("Capital", (eq, en) -> (eq instanceof WeaponType) && ((WeaponType) eq).isCapital(), Entity::isLargeCraft),
    ...
    INDUSTRIAL("Industrial", (eq, en) -> isIndustrialEquipment(eq), e -> (e instanceof Tank) || e.isSupportVehicle()),
    ...
    OTHER("Other",
          (eq, en) -> ((eq instanceof MiscType) &&
                             ...
                             !(isIndustrialEquipment(eq) &&
                                     ((en instanceof Tank) || en.isSupportVehicle() || en instanceof Mek)) &&
                             !eq.isAnyOf(LAM_FUEL_TANK, MECHANICAL_JUMP_BOOSTER) &&
                             !eq.hasFlag(F_TSM) &&
                             !eq.hasFlag(F_INDUSTRIAL_TSM) &&
                             !(eq.hasFlag(F_MASC) && !eq.hasSubType(S_SUPERCHARGER) && !eq.hasSubType(S_JETBOOSTER)) &&
                             ...
                             !(en.isSupportVehicle() &&
                                     (eq.hasFlag(F_BASIC_FIRECONTROL) || (eq.hasFlag(F_ADVANCED_FIRECONTROL)))) &&
    ...
    AP("Anti-Personnel", (eq, en) -> BattleArmorUtil.isBattleArmorAPWeapon(eq), e -> e instanceof BattleArmor),
    ...
    ONE_SHOT("One-Shot", (eq, en) -> (eq instanceof WeaponType) && eq.hasFlag(WeaponType.F_ONESHOT)),
    ...
    UNUSABLE_AMMO("Ammo w/o Weapon",
          (eq, en) -> (eq instanceof AmmoType) &&
                            !(eq instanceof BombType) &&
                            !eq.is(COOLANT_POD) &&
                            !UnitUtil.canUseAmmo(en, (AmmoType) eq, false));

    private final static Set<EquipmentDatabaseCategory> showFilters = EnumSet.of(ENERGY,
          BALLISTIC,
          MISSILE,
          ARTILLERY,
          CAPITAL,
    ...
    private final static Set<EquipmentDatabaseCategory> hideFilters = EnumSet.of(PROTOTYPE, AP, ONE_SHOT, UNAVAILABLE);
    ...
    public static boolean isIndustrialEquipment(EquipmentType equipment) {
        return equipment.isAnyOf(BACKHOE,
              LIGHT_BRIDGE_LAYER,
              MEDIUM_BRIDGE_LAYER,
              HEAVY_BRIDGE_LAYER,
              BULLDOZER,
              ...
              LADDER,
              LIFT_HOIST,
              MANIPULATOR_INDUSTRIAL,
              MINING_DRILL,
              NAIL_RIVET_GUN,
              REFUELING_DROGUE,
              FLUID_SUCTION_LIGHT_MEK,
              FLUID_SUCTION_LIGHT_VEE,
              FLUID_SUCTION,
              ROCK_CUTTER,
              SALVAGE_ARM,
              SPOT_WELDER,
