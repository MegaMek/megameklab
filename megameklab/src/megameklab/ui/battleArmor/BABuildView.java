...
/**
 * A component that displays a table listing all the unallocated equipment for the squad and allows dragging of the
 * equipment to critical slots to mount it.
 *
 * @author Taharqa
 * @author arlith
...
public class BABuildView extends IView implements ActionListener, MouseListener {
    ...
    public void mousePressed(MouseEvent e) {
        ...
        // locations, but only if those locations make sense
        if (e.getButton() == MouseEvent.BUTTON3) {
            JPopupMenu popup = new JPopupMenu();
            JMenuItem item;

            final int selectedRow = equipmentTable.rowAtPoint(e.getPoint());
            final Mounted<?> eq = (Mounted<?>) equipmentTable.getModel()
            ...
            if ((getBattleArmor() != null) && (eq.getType() instanceof AmmoType at)) {
                int maxNumShots = 4;
                int stepSize = 1;
    ...
    private void mountEquipmentInLocation(int location, int selectedRow) {
        ...
        // go back up to the grandparent build tab and fire a full refresh.
        ((BABuildTab) getParent().getParent()).refreshAll();
    }
}
