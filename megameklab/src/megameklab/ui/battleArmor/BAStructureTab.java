...
public class BAStructureTab extends ITab
    ...
    private final SpinnerNumberModel spnLeftManipulatorSizeModel = new SpinnerNumberModel(0.5,
          0.5,
          Double.MAX_VALUE,
          0.5);
    private final SpinnerNumberModel spnRightManipulatorSizeModel = new SpinnerNumberModel(0.5,
          0.5,
          Double.MAX_VALUE,
          0.5);
    ...
    public void setUpPanels() {
        ...
        panEnhancements = new BAEnhancementView();
        ...
        gbc.weightx = 1.0;                         // Expand to fill the horizontal space
    ...
    private void refreshManipulatorSizes(int mountLoc, JSpinner spinner, SpinnerNumberModel model) {
        Optional<MiscMounted> mounted = getBattleArmor().getMisc()
                                              .stream()
                                              .filter(m -> m.getType().hasFlag(MiscType.F_BA_MANIPULATOR) &&
                                                                 (m.getBaMountLoc() == mountLoc))
                                              .findFirst();
        ...
        if (e.getSource() instanceof JComboBox) {
            if (e.getSource().equals(leftManipulatorSelect) || e.getSource().equals(rightManipulatorSelect)) {
                ...
                setManipulator(BAManipulator.getManipulator(name),
                      e.getSource().equals(leftManipulatorSelect) ?
    ...
    private void setManipulator(BAManipulator manipulator, int mountLoc, boolean checkPaired) {
        ...
        if (manipulator != BAManipulator.NONE) {
            ...
            try {
                ...
                logger.error(ex, "Could not mount {}", manipulator);
            }
        }
        if (checkPaired) {
    ...
    public void stateChanged(ChangeEvent evt) {
        if (evt.getSource() == spnLeftManipulatorSize) {
            ...
            BAManipulator leftManipulator = BAManipulator.getManipulator(getBattleArmor().getLeftManipulatorName());
            if (leftManipulator != null && leftManipulator.pairMounted) {
    ...
    public void enhancementChanged(EquipmentType eq, boolean selected) {
        if (selected) {
            ...
            List<Mounted<?>> mounts = getBattleArmor().getMisc()
                                            .stream()
    ...
    public void useRemainingTonnageArmor() {
        ...
        int maxArmor = MathUtility.clamp(getBattleArmor().getMaximumArmorPoints(),
              0,
