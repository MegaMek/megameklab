...
public class CVMainUI extends MegaMekLabMainUI {
    ...
    public void createNewUnit(long entityType, boolean isPrimitive, boolean isIndustrial, Entity oldEntity) {
        ...
        newUnit.setEngine(new Engine(Math.max(10, (int) newUnit.getWeight() - newUnit.getSuspensionFactor()),
        ...
        if (null == oldEntity) {
            ...
            newUnit.setWeight(Math.min(newUnit.getWeight(),
                  TestTank.maxTonnage(newUnit.getMovementMode(), newUnit.isSuperHeavy())));
            if (newUnit.isSuperHeavy()) {
                newUnit.setWeight(Math.max(newUnit.getWeight(),
                      TestTank.maxTonnage(newUnit.getMovementMode(), false) + 1.0));
            }
        }
        newUnit.setOriginalWalkMP((newUnit.getEngine().getRating() + newUnit.getSuspensionFactor()) /
                                        (int) newUnit.getWeight());
        setEntity(newUnit, "");
