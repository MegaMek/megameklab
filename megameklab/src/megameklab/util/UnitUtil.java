...
public class UnitUtil {
    ...
    public static void removeMounted(Entity unit, Mounted<?> mount) {
        ...
        if (unit instanceof BattleArmor) {
            ...
            // If we are removing a weapon mounted in an DWP, we need to clear the mounted status of the DWP
            ...
            // If we're removing a weapon mounted in an APM, we need to clear the mounted status of the AP Mount.
        ...
        // Some special checks for Aerospace
        if (unit instanceof Aero) {
            if (mount instanceof WeaponMounted) {
                // Aerospace has additional weapon lists that need to be cleared
        ...
        // be set. We should remove it. Using getLinked could be unreliable, so we'll brutely force it. An example of this
            ...
            for (int slot = 0; slot < unit.getNumberOfCriticals(loc); slot++) {
                ...
                if ((cs != null) && (cs.getType() == CriticalSlot.TYPE_EQUIPMENT)) {
                    if (cs.getMount().equals(eq)) {
                        // If there are two pieces of equipment in this slot, remove the first one and replace it with the
                        // second
        ...
        for (int slot = 0; slot < unit.getNumberOfCriticals(loc); slot++) {
            ...
            if ((cs != null) && (cs.getType() == CriticalSlot.TYPE_EQUIPMENT)) {
                if (cs.getMount().equals(eq)) {
                    ...
                    // remove the first one and replace it with the second
                    if (cs.getMount2() != null) {
                        cs.setMount(cs.getMount2());
                        cs.setMount2(null);
                    } else {
    ...
    /**
     * Checks to see if the unit can use the tech level
     *
     * @param unit The entity
     *
    ...
    /**
     * Changes the location for a Mounted instance. Note: for BattleArmor, this affects which suit the equipment is
     * placed on (as that is what Mounted. Location means for BA), but not where on the suit it's located (i.e.,
     * BAMountLocation isn't affected). BattleArmor should change this outside this method.
     *
     ...
     * has not been assigned locations.
     *
     * @param unit The entity
     * @param eq   The equipment to test
     */
    ...
    public static double getMaximumArmorTonnage(Entity unit) {
        ...
        if (unit instanceof Mek) {
            ...
            // Add in extra armor points for the head
            if (unit.isSuperHeavy()) {
                points += 4;
            } else {
                points += 3;
            ...
            // Max armor is determined by the number of points.
    ...
    public static void updateCritsArmoredStatus(Entity unit, Mounted<?> mount) {
        /*
         * Several types of equipment have multiple fixed locations. These are always mounted in the primary location
         *  and added to critical slots in the other location(s). Examples are partial wing (both side torsos) and
         * Mek tracks (all legs). Rather than dealing with each piece of equipment individually and risking missing
     ...
     * the target location that does not match the source location is removed and assigned to Entity.LOC_NONE.
     *
     ...
     * it removes all ammo that has no location (which is how hidden ammo is kept when a unit is loaded from a file) but
    ...
    public static void checkEquipmentByTechLevel(Entity unit) {
        ...
        if (unit instanceof Infantry infantry) {
            if ((null != infantry.getPrimaryWeapon()) && !UnitUtil.isLegal(unit, infantry.getPrimaryWeapon())) {
                InfantryUtil.replaceMainWeapon(infantry,
                      (InfantryWeapon) EquipmentType.get("Infantry Auto Rifle"),
                      false);
            }
            if ((null != infantry.getSecondaryWeapon()) && !UnitUtil.isLegal(unit, infantry.getSecondaryWeapon())) {
                InfantryUtil.replaceMainWeapon(infantry, null, true);
            }
        }
    }
    ...
    public static boolean checkEquipmentByTechLevel(Entity unit, ITechManager techManager) {
        ...
        if (unit instanceof megamek.common.Infantry infantry) {
            if ((null != infantry.getPrimaryWeapon()) && !techManager.isLegal(infantry.getPrimaryWeapon())) {
                dirty = true;
                InfantryUtil.replaceMainWeapon(infantry,
                      (InfantryWeapon) EquipmentType.get("Infantry Auto Rifle"),
                      false);
            }
            if ((null != infantry.getSecondaryWeapon()) && !techManager.isLegal(infantry.getSecondaryWeapon())) {
                dirty = true;
                InfantryUtil.replaceMainWeapon(infantry, null, true);
            }
            if (techManager.getTechLevel().ordinal() <= SimpleTechLevel.STANDARD.ordinal() &&
                      infantry.hasFieldWeapon()) {
                InfantryUtil.replaceFieldGun(infantry, null, 0);
            }
        }
        return dirty;
    }
     ...
     * size (20 m for Ladders, 0.5t for Cargo space and the like).
     */
