        ...
        if ((eq.getType().isSpreadable() || eq.isSplitable()) && (totalCrits > 1)) {
            ...
            if ((eq.getType() instanceof WeaponType) && eq.getType().hasFlag(WeaponType.F_VGL)) {
                ...
                if (location == Mech.LOC_LT) {
                    ...
                    JOptionPane.showMessageDialog(null, "VGL must be placed in torso location!", "Invalid location",
                            JOptionPane.WARNING_MESSAGE);
                    return false;
                }
                String facing = (String) JOptionPane.showInputDialog(null, "Please choose the facing of the VGL",
    ...
    private boolean addEquipmentAero(Aero aero, Mounted eq) throws LocationFullException {
        if (eq.getType() instanceof WeaponType) {
            ...
            if ((weapCount[location] + 1) > availSpace[location]) {
                throw new LocationFullException(eq.getName() + " does not fit in " + getUnit().getLocationAbbr(location)
    ...
    public boolean importData(TransferSupport info) {
        ...
        if (info.getComponent() instanceof DropTargetCriticalList) {
            ...
            try {
                ...
                /*
                 * if (UnitUtil.isFixedLocationSpreadEquipment(eq.getType())) { return false; }
                 */

                if (!UnitUtil.isValidLocation(getUnit(), eq.getType(), location)) {
                    JOptionPane.showMessageDialog(null,
                            eq.getName() + " can't be placed in " + getUnit().getLocationName(location) + "!",
