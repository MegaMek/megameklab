...
public class PrintInfantry implements Printable {
	...
	public void printImage(Graphics2D g2d, PageFormat pageFormat) {
		...
		try {
			...
			for (int pos = 0; pos < stop; pos++) {
				diagram = ImageHelper
						.loadSVGImage(new File("data/images/recordsheets/Conventional_Infantry_platoon_" + (pos + 1) + ".svg"));
				...
				if (gun == null) {
					...
					((Text) tspan.getParent()).rebuild();
					/*
					 * We don't use StringUnits.getEquipmentInfo() to format the damage string
					 * because gauss explosion flags do not apply, and switchable only applies for
					 * non-LBX.
					 */
				...
				if (infantry.getPrimaryWeapon().hasFlag(WeaponType.F_INFERNO)
						|| (infantry.getSecondaryWeapon() != null && infantry.getSecondaryWeapon().hasFlag(WeaponType.F_INFERNO))) {
				...
				if (infantry.getPrimaryWeapon().hasFlag(WeaponType.F_INF_AA)
						|| (infantry.getSecondaryWeapon() != null && infantry.getSecondaryWeapon().hasFlag(WeaponType.F_INF_AA))) {
	...
	private static final int[][] RANGE_MODS = { { 0 }, { -2, 0, 2, 4 }, { -2, 0, 0, 2, 2, 4, 4 },
			{ -2, 0, 0, 0, 2, 2, 2, 4, 4, 4 }, { -2, 0, 0, 0, 0, 1, 1, 2, 2, 3, 3, 4, 4 },
			{ -1, 0, 0, 0, 0, 0, 1, 1, 2, 2, 2, 3, 3, 4, 4, 4 }, { -1, 0, 0, 0, 0, 0, 0, 1, 1, 1, 2, 2, 2, 4, 4, 4, 5, 5, 5 },
	 ...
	 * @param singleSecondary - secondary weapon if there is exactly one, otherwise
	 *                        null. This is used to account for point blank or
	 *                        encumbering penalties when the secondary weapon is not
	 *                        the basis for range mods.
	 * @param underwater      - whether the base range should be halved for
	 *                        underwater use by SCUBA platoons.
