...
public class EquipmentTab extends ITab implements ActionListener {
    ...
    private void removeEmptyBays() {
        List<Mounted> emptyBays = getAero().getWeaponBayList().stream().filter(bay -> bay.getBayWeapons().isEmpty())
            ...
            public boolean include(Entry<? extends EquipmentTableModel, ? extends Integer> entry) {
                ...
                if (etype instanceof WeaponType) {
                    ...
                    if ((wtype.isCapital() || (wtype.getAmmoType() == AmmoType.T_SCREEN_LAUNCHER))
                            && (nType != T_CAPITAL) && (nType != T_WEAPON)) {
                        return false;
                    }
                }
                AmmoType atype = null;
                ...
                if ((etype instanceof MiscType)
                        && (etype.hasFlag(MiscType.F_TSM) || etype.hasFlag(MiscType.F_INDUSTRIAL_TSM)
                                || (etype.hasFlag(MiscType.F_MASC) && !etype.hasSubType(MiscType.S_SUPERCHARGER)))) {
                    return false;
                }
                Aero aero = getAero();
                if (((nType == T_OTHER) && UnitUtil.isAeroEquipment(etype, getAero()))
                        || (((nType == T_WEAPON) && UnitUtil.isAeroWeapon(etype, aero)
                                && !(etype instanceof BayWeapon)))
                        || ((nType == T_ENERGY) && UnitUtil.isAeroWeapon(etype, aero) && (wtype != null)
                                && (wtype.hasFlag(WeaponType.F_ENERGY) || (wtype.hasFlag(WeaponType.F_PLASMA)
                                        && (wtype.getAmmoType() == AmmoType.T_PLASMA))))
                        || ((nType == T_BALLISTIC) && UnitUtil.isAeroWeapon(etype, aero) && (wtype != null)
                                && (wtype.hasFlag(WeaponType.F_BALLISTIC) && (wtype.getAmmoType() != AmmoType.T_NA)))
                        || ((nType == T_MISSILE) && UnitUtil.isAeroWeapon(etype, aero) && (wtype != null)
                                && ((wtype.hasFlag(WeaponType.F_MISSILE) && (wtype.getAmmoType() != AmmoType.T_NA))
                                        || (wtype.getAmmoType() == AmmoType.T_C3_REMOTE_SENSOR)))
                        || ((nType == T_CAPITAL) && UnitUtil.isAeroWeapon(etype, aero) && (wtype != null)
                                && (wtype.isCapital() || (wtype.getAmmoType() == AmmoType.T_SCREEN_LAUNCHER)))
                        || ((nType == T_ARTILLERY) && UnitUtil.isAeroWeapon(etype, aero) && (wtype != null)
