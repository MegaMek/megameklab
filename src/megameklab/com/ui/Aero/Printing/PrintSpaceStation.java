...
public class PrintSpaceStation implements Printable {

    private SpaceStation spacestation = null;
    private ArrayList<SpaceStation> spacestationList;
    PrinterJob masterPrintJob;

    public PrintSpaceStation(ArrayList<SpaceStation> list, PrinterJob masterPrintJob) {
        spacestationList = list;
    ...
    public void printImage(Graphics2D g2d, PageFormat pageFormat) {
        ...
        g2d.drawImage(ImageHelper.getRecordSheet(spacestation), 18, 18, 558, 738, Color.BLACK, null);
        printSmallCraftAerodyneImage(g2d, ImageHelper.getFluffImage(spacestation, ImageHelper.imageAero));
        ...
        // Armor Pips
        printFrontArmor(g2d, spacestation.getOArmor(Aero.LOC_NOSE));
        printLeftArmor(g2d, spacestation.getOArmor(Aero.LOC_LWING));
        printRightArmor(g2d, spacestation.getOArmor(Aero.LOC_RWING));
        printRearArmor(g2d, spacestation.getOArmor(Aero.LOC_AFT));

        // Internal Pips
        printStruct(g2d, spacestation.get0SI());
    ...
    private void printSmallCraftAerodyneData(Graphics2D g2d) {
        String smallCraftName = spacestation.getChassis() + " " + spacestation.getModel();
        ...
        if ((spacestation.getCrew() != null) && !spacestation.getCrew().getName().equalsIgnoreCase("unnamed")) {
            Crew pilot = spacestation.getCrew();
        ...
        g2d.drawString(Integer.toString(spacestation.getWalkMP()), 99, 143);
        g2d.drawString(Integer.toString(spacestation.getRunMP()), 99, 154);

        int tonnage = (int) Math.ceil(spacestation.getWeight());
        ...
        switch (spacestation.getTechLevel()) {
        ...
        if (spacestation.isMixedTech()) {
            if (spacestation.isClan()) {
                techBase = "Mixed Tech (Clan)";
            } else {
                techBase = "Mixed Tech (I.S.)";
            }
        } else if (spacestation.isClan()) {
            techBase = "Clan";
        }
        g2d.drawString(techBase, 177, 143.5f);

        if ((spacestation.getSource() != null) && (spacestation.getSource().trim().length() > 0)) {
            ...
            font = UnitUtil.getNewFont(g2d, spacestation.getSource(), false, 51, 8.0f);

            g2d.setFont(font);

            g2d.drawString(spacestation.getSource(), 177, nextDataLine);
            ...
            g2d.drawString(String.format("%1$s", spacestation.getYear()), 177, nextDataLine);
        ...
        // unusualSymbols);
        double bv = spacestation.calculateBattleValue(true, true);
        if (bv != -1) {
            ...
            g2d.setFont(font);
            g2d.drawString(String.format("%1$,d", spacestation.calculateBattleValue(true, true)), 50, 346.2f);
        }
    ...
    private void printArmor(Graphics2D g2d) {
        ...
        ImageHelper.printCenterString(g2d, String.format("%1$S (%2$s)", spacestation.getThresh(Aero.LOC_NOSE), spacestation.getArmor(Aero.LOC_NOSE)), g2d.getFont(), 442, 93);

        ImageHelper.printCenterString(g2d, String.format("%1$S (%2$s)", spacestation.getThresh(Aero.LOC_RWING), spacestation.getArmor(Aero.LOC_RWING)), g2d.getFont(), 517, 310);

        ImageHelper.printCenterString(g2d, String.format("%1$S (%2$s)", spacestation.getThresh(Aero.LOC_LWING), spacestation.getArmor(Aero.LOC_LWING)), g2d.getFont(), 270, 310);

        ImageHelper.printCenterString(g2d, String.format("%1$S (%2$s)", spacestation.getThresh(Aero.LOC_AFT), spacestation.getArmor(Aero.LOC_AFT)), g2d.getFont(), 447, 487);

        g2d.drawString(String.format("%1$S", spacestation.get0SI()), 390, 268);
    }
    ...
    private void printHeatSinks(Graphics2D g2d) {
        ...
        // Heat Sinks
        if (spacestation.getHeatType() == Aero.HEAT_DOUBLE) {
            g2d.drawString(String.format("%1$s (%2$s)", spacestation.getHeatSinks(), spacestation.getHeatSinks() * 2), 502, 535);
            g2d.drawString("Double", 502, 543);
        } else {
            g2d.drawString(String.format("%1$s (%1$s)", spacestation.getHeatSinks()), 502, 535);
        ...
        for (int pos = 1; pos <= spacestation.getHeatSinks(); pos++) {
    ...
    private void printWeaponsNEquipment(Graphics2D g2d) {
        ...
        ImageHelperAero.printAeroWeaponsNEquipment(spacestation, g2d);
    ...
    public void print(HashPrintRequestAttributeSet aset) {
        ...
        try {
            for (int pos = 0; pos < spacestationList.size(); pos++) {
                ...
                spacestation = spacestationList.get(pos);
                pj.setJobName(spacestation.getChassis() + " " + spacestation.getModel());
