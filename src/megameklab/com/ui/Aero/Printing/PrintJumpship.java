...
public class PrintJumpship implements Printable {

    private Jumpship jumpship = null;
    private ArrayList<Jumpship> jumpshipList;
    PrinterJob masterPrintJob;

    public PrintJumpship(ArrayList<Jumpship> list, PrinterJob masterPrintJob) {
        jumpshipList = list;
    ...
    public void printImage(Graphics2D g2d, PageFormat pageFormat) {
        ...
        g2d.drawImage(ImageHelper.getRecordSheet(jumpship), 18, 18, 558, 738, Color.BLACK, null);
        printSmallCraftAerodyneImage(g2d, ImageHelper.getFluffImage(jumpship, ImageHelper.imageAero));
        ...
        // Armor Pips
        printFrontArmor(g2d, jumpship.getOArmor(Aero.LOC_NOSE));
        printLeftArmor(g2d, jumpship.getOArmor(Aero.LOC_LWING));
        printRightArmor(g2d, jumpship.getOArmor(Aero.LOC_RWING));
        printRearArmor(g2d, jumpship.getOArmor(Aero.LOC_AFT));

        // Internal Pips
        printStruct(g2d, jumpship.get0SI());
    ...
    private void printSmallCraftAerodyneData(Graphics2D g2d) {
        String smallCraftName = jumpship.getChassis() + " " + jumpship.getModel();
        ...
        if ((jumpship.getCrew() != null) && !jumpship.getCrew().getName().equalsIgnoreCase("unnamed")) {
            Crew pilot = jumpship.getCrew();
        ...
        g2d.drawString(Integer.toString(jumpship.getWalkMP()), 99, 143);
        g2d.drawString(Integer.toString(jumpship.getRunMP()), 99, 154);

        int tonnage = (int) Math.ceil(jumpship.getWeight());
        ...
        if (jumpship.isMixedTech()) {
            if (jumpship.isClan()) {
                techBase = "Mixed Tech (Clan)";
            } else {
                techBase = "Mixed Tech (I.S.)";
            }
        } else if (jumpship.isClan()) {
            techBase = "Clan";
        }
        g2d.drawString(techBase, 177, 143.5f);

        if ((jumpship.getSource() != null) && (jumpship.getSource().trim().length() > 0)) {
            ...
            font = UnitUtil.getNewFont(g2d, jumpship.getSource(), false, 51, 8.0f);

            g2d.setFont(font);

            g2d.drawString(jumpship.getSource(), 177, nextDataLine);
            ...
            g2d.drawString(String.format("%1$s", jumpship.getYear()), 177, nextDataLine);
        ...
        // unusualSymbols);
        double bv = jumpship.calculateBattleValue(true, true);
        if (bv != -1) {
            ...
            g2d.setFont(font);
            g2d.drawString(String.format("%1$,d", jumpship.calculateBattleValue(true, true)), 50, 346.2f);
        }
    ...
    private void printArmor(Graphics2D g2d) {
        ...
        ImageHelper.printCenterString(g2d, String.format("%1$S (%2$s)", jumpship.getThresh(Aero.LOC_NOSE), jumpship.getArmor(Aero.LOC_NOSE)), g2d.getFont(), 442, 93);

        ImageHelper.printCenterString(g2d, String.format("%1$S (%2$s)", jumpship.getThresh(Aero.LOC_RWING), jumpship.getArmor(Aero.LOC_RWING)), g2d.getFont(), 517, 310);

        ImageHelper.printCenterString(g2d, String.format("%1$S (%2$s)", jumpship.getThresh(Aero.LOC_LWING), jumpship.getArmor(Aero.LOC_LWING)), g2d.getFont(), 270, 310);

        ImageHelper.printCenterString(g2d, String.format("%1$S (%2$s)", jumpship.getThresh(Aero.LOC_AFT), jumpship.getArmor(Aero.LOC_AFT)), g2d.getFont(), 447, 487);

        g2d.drawString(String.format("%1$S", jumpship.get0SI()), 390, 268);
    }
    ...
    private void printHeatSinks(Graphics2D g2d) {
        ...
        // Heat Sinks
        if (jumpship.getHeatType() == Aero.HEAT_DOUBLE) {
            g2d.drawString(String.format("%1$s (%2$s)", jumpship.getHeatSinks(), jumpship.getHeatSinks() * 2), 502, 535);
            g2d.drawString("Double", 502, 543);
        } else {
            g2d.drawString(String.format("%1$s (%1$s)", jumpship.getHeatSinks()), 502, 535);
        ...
        for (int pos = 1; pos <= jumpship.getHeatSinks(); pos++) {
    ...
    private void printWeaponsNEquipment(Graphics2D g2d) {
        ...
        ImageHelperAero.printAeroWeaponsNEquipment(jumpship, g2d);
    ...
    public void print(HashPrintRequestAttributeSet aset) {
        ...
        try {
            for (int pos = 0; pos < jumpshipList.size(); pos++) {
                ...
                jumpship = jumpshipList.get(pos);
                pj.setJobName(jumpship.getChassis() + " " + jumpship.getModel());
