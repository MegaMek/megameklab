...
/**
 * Panel for assigning armor type and tonnage for mechs, (combat) vehicles, and
 * fighters.
 *
 * @author Neoancient
 *
...
public class MVFArmorView extends BuildView implements ActionListener, ChangeListener {
    ...
    /**
     * Sets the values of all fields from the current Entity, with the option of
     * ignoring whether the {@code Entity} has patchwork armor. This is because the
     * Entity does not report patchwork armor unless it actually has multiple armor
     * types, and we don't want to clear the patchwork checkbox unless we're loading
     * a new unit.
     *
     * @param en                The Entity being edited
     * @param overridePatchwork Whether to ignore whether the Entity has patchwork
     *                          armor.
     */
    ...
    public void actionPerformed(ActionEvent e) {
        if ((e.getSource() == cbArmorType) || ((e.getSource() == chkPatchwork) && !isPatchwork())) {
            ...
            listeners.forEach(
                    l -> l.armorTypeChanged(EquipmentType.T_ARMOR_PATCHWORK, Entity.getPatchworkArmorAdvancement()
