...
/**
 * Panel for conventional infantry weapons (primary, secondary, field gun). The
 * only editable controls are for changing the number of secondary weapons and
 * field guns.
 *
 * @author Neoancient
 *
...
public class InfantryWeaponView extends BuildView implements ActionListener {
    ...
    private final EnumSet<EntityMovementMode> FIELD_GUN_MODES = EnumSet.of(EntityMovementMode.TRACKED,
            EntityMovementMode.WHEELED, EntityMovementMode.INF_MOTORIZED);
    ...
    public void setFromEntity(Infantry inf) {
        ...
        List<EquipmentType> fieldGuns = inf.getWeaponList().stream()
                ...
                .filter(et -> et instanceof WeaponType).collect(Collectors.toList());
