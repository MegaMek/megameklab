        ...
        equipmentTypeFilter = new RowFilter<EquipmentTableModel, Integer>() {
            ...
            public boolean include(Entry<? extends EquipmentTableModel, ? extends Integer> entry) {
                ...
                if ((etype instanceof MiscType) && (etype.hasFlag(MiscType.F_TSM)
                        || etype.hasFlag(MiscType.F_INDUSTRIAL_TSM) || (etype.hasFlag(MiscType.F_SCM))
                        || (etype.hasFlag(MiscType.F_MASC) && !etype.hasSubType(MiscType.S_SUPERCHARGER)))) {
                    return false;
                }
                ...
                if (((nType == T_OTHER) && UnitUtil.isMechEquipment(etype, mech))
                        || (((nType == T_WEAPON)
                                && (UnitUtil.isMechWeapon(etype, mech) || UnitUtil.isPhysicalWeapon(etype))))
                        || ((nType == T_ENERGY) && UnitUtil.isMechWeapon(etype, mech) && (wtype != null)
                                && (wtype.hasFlag(WeaponType.F_ENERGY) || (wtype.hasFlag(WeaponType.F_PLASMA)
                                        && (wtype.getAmmoType() == AmmoType.T_PLASMA))))
                        || ((nType == T_BALLISTIC) && UnitUtil.isMechWeapon(etype, mech) && (wtype != null)
                                && (wtype.hasFlag(WeaponType.F_BALLISTIC) && (wtype.getAmmoType() != AmmoType.T_NA)))
                        || ((nType == T_MISSILE) && UnitUtil.isMechWeapon(etype, mech) && (wtype != null)
                                && ((wtype.hasFlag(WeaponType.F_MISSILE) && (wtype.getAmmoType() != AmmoType.T_NA))
                                        || (wtype.getAmmoType() == AmmoType.T_C3_REMOTE_SENSOR)))
                        || ((nType == T_ARTILLERY) && UnitUtil.isMechWeapon(etype, mech) && (wtype != null)
